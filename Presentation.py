...
✅ 주요 요약
 ...
 - **APEC 정상회의 공식 후원**  <br>
  우리금융이 아시아태평양경제협력체(APEC) 정상회의를 공식 후원.
  글로벌 인지도 제고 및 외교 무대에서 브랜드 가치 강화
...
.topbar-cta{
  ...
  box-shadow: inset 0 -2px 0 rgba(11,98,230,.06);
}
.topbar-cta:active { transform:translateY(-50%) scale(0.98); }

/* 평가금액 카드: 금액/안내문 좌측 정렬 */
.eval-amount{ text-align:left !important; }
.eval-note{   text-align:left !important; }
...
<div class="topbar-wrap"><div class="topbar">
  ...
  {f'<a class="topbar-cta" href="{HEADER_LINK[1]}" target="_blank" rel="noopener noreferrer">{HEADER_LINK[0]}</a>' if HEADER_LINK else ''}
</div></div>
...
<div class="phone-wrap"><div class="phone"><div class="card">
  ...
  <div class="eval-note" style="color:#9aa4b2;font-size:13px;margin-top:4px;">전날(영업일 기준) 기준, 오늘 입금액은 반영 안 됨</div>
