...
if "open_news_modal_once" not in st.session_state:
    st.session_state.open_news_modal_once = False
...
# 이 렌더에서만 오픈 여부 확정
OPEN_NEWS_MODAL_NOW = st.session_state.pop("open_news_modal_once", False)
...
TABS = [
    {
        ...
        "ticker":"우리금융지주",
        "label":"RISE 코리아금융고배당",
        "components": ["우리금융지주","삼성화재","삼성생명","카카오뱅크","메리츠금융지주"],   # 👈 구성종목 칩
        "content": """
✅ 주요 요약
...
.badge-new{ display:inline-block; margin-left:6px; padding:2px 6px; border-radius:999px; font-weight:900; font-size:11px; background:#ff5a65; color:#fff; }
 ...
 /* ===== 요약 카드 하단 '뉴스 보기' 버튼(카드 버튼과 동일 톤) ===== */
div[data-testid="stVerticalBlock"]:has(.summary-cta-scope){
  max-width:520px; 
  margin:-6px auto 8px !important;   /* 카드 안쪽으로 바짝 붙이기 */
  text-align:center;
}
...
.summary-tags{
  ...
  justify-content:flex-start;   /* 필요하면 space-between 으로 바꿔도 됨 */
  margin:6px 0 8px;
}
.summary-tag{
  ...
  padding:6px 10px;
  border-radius:999px;
  box-shadow: inset 0 -1px 0 rgba(11,98,230,.06);
}
